...
function DepartmentStudentData() {
  ...
  const [studentData, setStudentData] = useState<DeptStudentFormData | null>(
    null
  );
  ...
  const handleFetchStudent = async (
    ...
    try {
      const response = await axios.get(
        `/api/department_coordinator/student-data/?uid=${uidInput}`,
        {
          headers: {
    ...
    <Box maxWidth="1200px" mx="auto" p={4}>
      ...
      {studentData && (
        <Stack spacing={3}>
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} md={6}>
                  <Stack spacing={1}>
                    ...
                    <InfoRow
                      label="Category"
                      value={studentData.current_category}
                    />
            ...
            <CardContent>
              ...
              <Grid container spacing={3}>
                <Grid item xs={6} sm={3}>
                  <MetricCard
                    label="CGPA"
                    value={parseFloat(studentData.cgpa?.toFixed(2) ?? "0")}
                    max={10}
                  />
                </Grid>
                <Grid item xs={6} sm={3}>
                ...
                <Grid item xs={6} sm={3}>
                  <MetricCard
                    label="12th Grade"
                    value={studentData.higher_secondary_grade}
                    max={100}
                    unit="%"
                  />
                </Grid>
              ...
              <Stack direction="row" spacing={2} mt={3} flexWrap="wrap">
                ...
                <StatusChip
                  label={
                    studentData.joined_company ? "Joined Company" : "Not Joined"
                  }
                  active={studentData.joined_company}
                />
                <StatusChip
                  label={
                    studentData.is_dse_student
                      ? "DSE Student"
                      : "Regular Student"
                  }
                ...
                <Stack spacing={2}>
                  {studentData.offers.map((offer: any, index: number) => (
                    <Paper key={index} variant="outlined" sx={{ p: 2 }}>
                      ...
                      <Stack direction="row" spacing={2}>
                        <Chip
                          label={`₹${(offer.salary / 100000).toFixed(1)}L`}
                          size="small"
                          variant="outlined"
                        />
                        <Chip
                      ...
                      >
                        ...
                        {app.progress?.final_result && (
                          <Chip
                            ...
                            size="small"
                            color={
                              app.progress.final_result === "Selected"
                                ? "success"
                                : "default"
                            }
                          />
                        ...
                        {app.progress && (
                          <Box>
                            ...
                            <Grid container spacing={1}>
                              <ProgressItem
                                label="Registered"
                                completed={app.progress.registered}
                              />
                              <ProgressItem
                                label="Aptitude Test"
                                completed={app.progress.aptitude_test}
                              />
                              <ProgressItem
                                label="Coding Test"
                                completed={app.progress.coding_test}
                              />
                              <ProgressItem
                                label="Technical Interview"
                                completed={app.progress.technical_interview}
                              />
                              <ProgressItem
                                label="HR Interview"
                                completed={app.progress.hr_interview}
                              />
                              <ProgressItem
                                label="GD"
                                completed={app.progress.gd}
                              />
                            </Grid>
                          </Box>
                        )}
          ...
          {studentData.training_performance &&
            studentData.training_performance.length > 0 && (
              <Card elevation={3}>
                <CardContent>
                  <Typography variant="h6" fontWeight="bold" mb={2}>
                    Training Performance
                  ...
                  <Stack spacing={2}>
                    {studentData.training_performance.map(
                              ...
                              <Table size="small">
                                ...
                                <TableBody>
                                  ...
                                  <TableRow>
                                    ...
                                    <TableCell align="right">
                                      <strong>{training.total_marks}</strong>
                                    </TableCell>
                                  </TableRow>
                                </TableBody>
                              </Table>
